...
The first run downloads the model package (17.4 GB), verifies it, checks
available memory, and starts serving on `127.0.0.1:8000`. When it prints its
`Ready` line, open <http://127.0.0.1:8000> or attach an agent you already have
...
`model` is the package ID you served, or omit it. Reasoning is on by default.
`"reasoning_effort": "none"` turns it off, and Qwen3.8-27B also takes `low`,
`medium`, and `xhigh`.
...
time. Context scales to the model's native 256K window within the memory left
after the weights. The only settings are ceilings that leave room for other work
on the same Mac:
