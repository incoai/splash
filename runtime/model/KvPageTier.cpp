...
uint64_t KvPageTier::slotBytesFor(const kv::Q8PageStorage &pages) noexcept {
  const uint64_t unit = SlotFile::kAlignmentBytes;
  return (pages.bytesPerPage() + unit - 1) / unit * unit;
}

KvPageTier::KvPageTier(metal::MetalBackend &backend, kv::Q8PageStorage &pages,
...
std::unique_ptr<KvTransfer> KvPageTier::demote(uint32_t page, std::shared_ptr<KvDiskSlot> slot,
  ...
  if (demotionsInFlight_ >= demotionSlots_ || freeStaging_.empty() || !file_->writable())
    return {};
  ++demotionsInFlight_;
  ...
  return std::make_unique<Ticket>(std::move(transfer));
}
