...
void KvCopy::addPages(metal::CommandGraph &graph, const kv::Q8PageStorage &pages,
                      metal::MetalBuffer staging, metal::MetalBuffer table,
                      uint32_t stagingSlots, uint64_t slotBytes) {
  const kv::Q8Layout layout = pages.layout();
  ...
  for (uint32_t layer = 0; layer < layout.attentionLayers; ++layer) {
    const kv::Q8LayerStorage &storage = pages.layer(layer);
    params.layer_offset = static_cast<uint32_t>(layer * layout.bytesPerLayerPage());
    graph.add("kv_copy_pages",
              {storage.keyData, storage.keyScales, storage.valueData, storage.valueScales,
               staging, table},
