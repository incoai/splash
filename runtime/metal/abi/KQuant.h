...
// Load-time repack of native GGUF rows into the MDKQ0001 planes (kq_repack):
// one thread per (destination row, 32-wide K group).
struct KQRepackParams {
  uint32_t rows;                // destination rows (multiple of 256)
  uint32_t input_size;          // K
  uint32_t fmt;                 // KQ_FMT_*
...
static_assert(sizeof(KQRepackParams) == 48, "K-quant repack parameters are 48 bytes on both sides");
...
static_assert(sizeof(KQCopyParams) == 12, "K-quant copy parameters are 12 bytes on both sides");
