...
// Fills the threadgroup table of format F's value pairs, entry b = (F::value(b & 15), F::value(b >> 4)) for the
// index byte b of a pair, when F is a codebook format; called by all threads of the threadgroup.
template <class F>
inline void quant_pair_table(threadgroup half2 *table, uint thread_index, uint threads) {
  if constexpr (F::Kind == QuantCodebook) {
    for (uint i = thread_index; i < kQuantPairTableEntries; i += threads) table[i] = half2(F::value(i & 15), F::value(i >> 4));
...
struct FmtQ6K {
  ...
  static QuantCoef coef(Meta mt, ushort j) {
    ...
    return {float2(d * float(as_type<char>(uchar(pair & 0xFFu))), d * float(as_type<char>(uchar(pair >> 8)))), float2(0.0f)};
  }
};
...
struct FmtQ3K {
  ...
  static QuantCoef coef(Meta mt, ushort j) {
    ...
    return {float2(d * float(int(pair & 0xFFu) - 32), d * float(int((pair >> 8) & 0xFFu) - 32)), float2(0.0f)};
  }
};
...
struct FmtIQ4XS {
  ...
  static QuantCoef coef(Meta mt, ushort j) {
    ...
    return {float2(float(d) * float(ls - 32)), float2(0.0f)};
  }
};
...
struct FmtIQ3S {
  ...
  static QuantCoef coef(Meta mt, Chunk q) { return {float2(float(as_type<half>(mt)) * float(1 + 2 * ((q >> 26) & 0xF))), float2(0.0f)}; }
};
...
inline QuantCoef quant_iq2_coef(ushort d, uint scales) {
  ...
  return {float2(s * float(1 + 2 * (scales & 0xF)) * 0.125f, s * float(1 + 2 * ((scales >> 4) & 0xF)) * 0.125f), float2(0.0f)};
}
...
struct FmtIQ3XXS {
  ...
  static uint signs(Chunk q) { return (q >> 16) & 0xFF; }
  static QuantCoef coef(Meta mt, Chunk q) { return {float2(float(as_type<half>(mt)) * float(1 + 2 * ((q >> 26) & 0xF)) * 0.25f), float2(0.0f)}; }
};
...
struct FmtIQ1S {
  ...
  static QuantCoef coef(Meta mt, Chunk q) {
    return {float2(float(as_type<half>(mt)) * float(2 * ((q >> 25) & 7) + 1) * 0.125f), float2(0.0f)};
  }
};
...
struct FmtIQ1M {
  ...
  static QuantCoef coef(Meta mt, ushort j) {
    ...
    return {float2(s * float(2 * (scales & 7) + 1) * 0.125f, s * float(2 * ((scales >> 3) & 7) + 1) * 0.125f), float2(0.0f)};
  }
};
...
inline void quant_format_switch(uint format, Body body) {
  ...
  case GGUF_FMT_Q2K: body(FmtQ2K()); break;
  case GGUF_FMT_IQ3XXS: body(FmtIQ3XXS()); break;
  case GGUF_FMT_IQ2XXS: body(FmtIQ2XXS()); break;
  case GGUF_FMT_IQ2XS: body(FmtIQ2XS()); break;
  case GGUF_FMT_IQ2S: body(FmtIQ2S()); break;
  case GGUF_FMT_IQ1S: body(FmtIQ1S()); break;
  case GGUF_FMT_IQ1M: body(FmtIQ1M()); break;
