...
void roundTrip(metal::MetalBackend &backend, engine::MemoryGovernor &governor,
               kv::Q8Layout layout) {
  const uint32_t batch = layout.sparseMappingBatchPages();
  kv::Q8PageStorage pages(backend, governor.allocationAdmission(), layout, 2 * batch);
  const uint32_t pageA = 7;
  const uint32_t pageB = batch + 3;
  ...
  std::cout << "round trip " << layout.attentionLayers << "x" << layout.kvHeads << "x"
...
void staging(metal::MetalBackend &backend, engine::MemoryGovernor &governor) {
  const kv::Q8Layout layout{2, 2, 256};
  kv::Q8PageStorage pages(backend, governor.allocationAdmission(), layout,
                          layout.sparseMappingBatchPages());
  const uint64_t slotBytes = KvPageTier::slotBytesFor(pages);
  const uint64_t payload = pages.bytesPerPage();
  ...
  require(!tier.restore(written, 8, {}), "a third transfer found staging that does not exist");
  ...
  require(!tier.demote(6, empty, {}), "demotions took the restores' half of the ring");
...
void teardown(metal::MetalBackend &backend, engine::MemoryGovernor &governor) {
  const kv::Q8Layout layout{16, 4, 256}; // Qwen3.6-27B: about a MiB per page
  kv::Q8PageStorage pages(backend, governor.allocationAdmission(), layout,
                          layout.sparseMappingBatchPages());
  const uint64_t slotBytes = KvPageTier::slotBytesFor(pages);
...
void run(const std::string &metallib) {
  ...
  roundTrip(backend, governor, kv::Q8Layout{2, 2, 256});
  roundTrip(backend, governor, kv::Q8Layout{10, 2, 256}); // Qwen3.6-35B
  roundTrip(backend, governor, kv::Q8Layout{16, 4, 256}); // Qwen3.6-27B
  staging(backend, governor);
  shares(backend, governor);
  teardown(backend, governor);
