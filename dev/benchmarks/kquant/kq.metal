...
struct QParams { uint output_size; uint input_size; uint persistent_groups; uint out_stride; uint out_offset; };
enum Epilogue : ushort { EpNone = 0, EpResidual = 1, EpUpWithGate = 2 };
...
inline void store_poly8(uint v, half2 s2, threadgroup half *dst) {   // kv(q) = rint(c4 q^4 + c3 q^3 + c2 q^2 + c1 q + c0), exact for q in 0..15
  const half2 k = half2(1024.0h); const float2 s = float2(s2); half4 o0, o1;
  const float2 q0 = float2(as_type<half2>(((v >> 0) & 0x000F000Fu) | 0x64006400u) - k), q1 = float2(as_type<half2>(((v >> 4) & 0x000F000Fu) | 0x64006400u) - k);
  const float2 q2 = float2(as_type<half2>(((v >> 8) & 0x000F000Fu) | 0x64006400u) - k), q3 = float2(as_type<half2>(((v >> 12) & 0x000F000Fu) | 0x64006400u) - k);
#define KQ_POLY(q) rint(fma(fma(fma(fma(float2(1.324720171e-03f), q, float2(4.121682411e-02f)), q, float2(-1.500183575e+00f)), q, float2(2.474324575e+01f)), q, float2(-1.270436017e+02f)))
  o0.xy = half2(KQ_POLY(q0) * s); o0.zw = half2(KQ_POLY(q1) * s); o1.xy = half2(KQ_POLY(q2) * s); o1.zw = half2(KQ_POLY(q3) * s);
#undef KQ_POLY
  *((threadgroup half4 *)dst) = o0; *((threadgroup half4 *)(dst + 4)) = o1;
}
...
#pragma unroll
    for (ushort k = 0; k < 4; ++k) {
      ...
      else if constexpr (Lut == 4) store_poly8(w.a[k], s2, dst + 8 * k);
...
#pragma unroll
  for (ushort i = 0; i < acc.get_capacity(); ++i) {
    ...
    if constexpr (Ep == EpUpWithGate) v *= silu_gate(float(aux[o]));
    output[o] = TO(v);
  }
  simdgroup_barrier(mem_flags::mem_threadgroup);
}
...
#define ABUF(TA) device TA *input [[buffer(0)]], device uchar *w0 [[buffer(1)]], device uchar *meta [[buffer(2)]], \
                 device bfloat *output [[buffer(3)]], constant QParams &p [[buffer(4)]], device uchar *w1 [[buffer(5)]]
...
#define SGK_K(F, f, TA, ta, R, C, S, KS, B, P)                                                            \
  kernel void sgk##ta##_##f##_m##R##_c##C##_sg##S##_k##KS##_b##B##_p##P(device TA *input [[buffer(0)]], device uchar *w0 [[buffer(1)]], \
             device uchar *meta [[buffer(2)]], device float *partials [[buffer(3)]], constant QParams &p [[buffer(4)]], device uchar *w1 [[buffer(5)]], \
...
struct ReduceParams { uint splits; uint rows; uint cols; uint out_stride; uint out_offset; uint epilogue; };
kernel void splitk_reduce(device float *partials [[buffer(0)]], device bfloat *output [[buffer(1)]], constant ReduceParams &rp [[buffer(2)]],
                          device bfloat *aux [[buffer(3)]], uint tid [[thread_position_in_grid]]) {
  ...
  if (rp.epilogue == EpUpWithGate) s *= silu_gate(float(aux[o]));
  output[o] = bfloat(s);
}
// epilogue entry points (bf16 activations): residual add or silu(gate)*acc, aux in buffer(6)
#define SGE_K(F, f, EP, ep, R, C, S, KS, B, P)                                                            \
  kernel void sg##ep##_##f##_m##R##_c##C##_sg##S##_k##KS##_b##B##_p##P(ABUF(bfloat), device bfloat *aux [[buffer(6)]], uint group [[threadgroup_position_in_grid]], IDS) { \
    constexpr ushort Threads = S * 32; TGLUT_INIT(F)                                                      \
    threadgroup half stage[S * B * KS * C]; const uint tiles = p.output_size / (S * C), steps = p.input_size / KS; \
    for (uint tile = group; tile < tiles; tile += p.persistent_groups)                                    \
...
#define PFE_K(F, f, EP, ep, R, S, N, KS, P)                                                               \
  kernel void pf##ep##_##f##_r##R##_sg##S##_n##N##_k##KS##_p##P(ABUF(bfloat), device bfloat *aux [[buffer(6)]], uint2 group [[threadgroup_position_in_grid]], IDS) { \
    constexpr ushort Threads = S * 32; TGLUT_INIT(F)                                                      \
...
#define DECODE_SET(F, f, TA, ta, R)                                                                       \
  SG_K(F, f, TA, ta, R, 32, 4, 32, 2, 1) SG_K(F, f, TA, ta, R, 32, 2, 32, 2, 1) SG_K(F, f, TA, ta, R, 32, 1, 32, 2, 1) \
  SG_K(F, f, TA, ta, R, 32, 4, 32, 1, 1) SG_K(F, f, TA, ta, R, 32, 2, 32, 2, 2) SG_K(F, f, TA, ta, R, 16, 8, 64, 1, 1) \
  SG_K(F, f, TA, ta, R, 64, 1, 32, 2, 1) SG_K(F, f, TA, ta, R, 32, 2, 32, 1, 1) SG_K(F, f, TA, ta, R, 32, 1, 32, 1, 1) \
  SG_K(F, f, TA, ta, R, 32, 2, 32, 1, 2) SG_K(F, f, TA, ta, R, 32, 1, 32, 1, 2) SG_K(F, f, TA, ta, R, 32, 2, 64, 1, 1) \
  SG_K(F, f, TA, ta, R, 32, 2, 64, 1, 2) SG_K(F, f, TA, ta, R, 32, 1, 64, 1, 2) SG_K(F, f, TA, ta, R, 32, 1, 32, 2, 2) \
  SG_K(F, f, TA, ta, R, 16, 4, 32, 1, 2) SG_K(F, f, TA, ta, R, 16, 2, 64, 1, 2) SG_K(F, f, TA, ta, R, 32, 1, 32, 2, 3) \
  SGK_K(F, f, TA, ta, R, 32, 4, 32, 2, 1) SGK_K(F, f, TA, ta, R, 32, 2, 32, 2, 1) SGK_K(F, f, TA, ta, R, 16, 8, 64, 1, 1)
#define PREFILL_SET(F, f, TA, ta)                                                                         \
  PF_K(F, f, TA, ta, 32, 4, 64, 64, 1) PF_K(F, f, TA, ta, 32, 4, 64, 32, 1) PF_K(F, f, TA, ta, 16, 8, 64, 64, 1) \
  PF_K(F, f, TA, ta, 32, 8, 64, 64, 1) PF_K(F, f, TA, ta, 16, 4, 128, 32, 1) PF_K(F, f, TA, ta, 16, 4, 64, 64, 1)
#define EPI_SET(F, f, EP, ep)                                                                             \
  SGE_K(F, f, EP, ep, 8, 32, 2, 32, 2, 1) SGE_K(F, f, EP, ep, 16, 32, 2, 32, 2, 1) SGE_K(F, f, EP, ep, 32, 32, 2, 32, 2, 1) \
  SGE_K(F, f, EP, ep, 8, 32, 4, 32, 2, 1) SGE_K(F, f, EP, ep, 16, 32, 4, 32, 2, 1) SGE_K(F, f, EP, ep, 32, 32, 4, 32, 2, 1) \
  PFE_K(F, f, EP, ep, 32, 4, 64, 64, 1) PFE_K(F, f, EP, ep, 32, 4, 64, 32, 1) PFE_K(F, f, EP, ep, 16, 4, 128, 32, 1)
#define ALL_KERNELS(F, f)                                                                                 \
  DECODE_SET(F, f, half, , 8) DECODE_SET(F, f, half, , 16) DECODE_SET(F, f, half, , 32) PREFILL_SET(F, f, half, ) \
  DECODE_SET(F, f, bfloat, a, 8) DECODE_SET(F, f, bfloat, a, 16) DECODE_SET(F, f, bfloat, a, 32) PREFILL_SET(F, f, bfloat, a) \
  EPI_SET(F, f, EpResidual, r) EPI_SET(F, f, EpUpWithGate, g)
ALL_KERNELS(FmtQ4K, q4k)
ALL_KERNELS(FmtIQ4XS<0>, iq4xs)
ALL_KERNELS(FmtIQ4XS<2>, iq4xsB)
ALL_KERNELS(FmtIQ4XS<3>, iq4xsT)
ALL_KERNELS(FmtIQ4XS<4>, iq4xsP)
ALL_KERNELS(FmtIQ4NL<0>, iq4nl)
ALL_KERNELS(FmtIQ4NL<2>, iq4nlB)
ALL_KERNELS(FmtQ5K, q5k)
ALL_KERNELS(FmtQ6K, q6k)
ALL_KERNELS(FmtQ3K, q3k)
ALL_KERNELS(FmtQ80, q80)
ALL_KERNELS(FmtIQ3S, iq3s)

// ---------------- token embedding gather from native block_q4_K rows (row = K/256 blocks of 144 B)
struct EmbedParams { uint rows; uint vocabulary; uint hidden; };
kernel void embed_q4k(device const uint *tokens [[buffer(0)]], device const uchar *table [[buffer(1)]], device bfloat *output [[buffer(2)]],
                      constant EmbedParams &p [[buffer(3)]], uint index [[thread_position_in_grid]]) {
...
struct PermuteParams { uint rows; uint width; uint block; };
kernel void permute_heads(device const bfloat *input [[buffer(0)]], device bfloat *output [[buffer(1)]], device const uint *perm [[buffer(2)]],
                          constant PermuteParams &p [[buffer(3)]], uint index [[thread_position_in_grid]]) {
...
// dependency kernel: touching the output forces the next dispatch to wait (no overlap between consecutive GEMMs)
